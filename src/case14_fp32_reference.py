...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "also run the dispatcher's FP32-facing mixed Case-14 route on each "
            "sample and apply the official criterion"
        ),
    )
...
def run(args: argparse.Namespace) -> int:
    """Run from this module's CLI or the Case-14 branch in ``src.benchmark``."""
    ...
    if args.compare_current_candidate:
        ...
        # Transformer inference, and the validation entrypoint accepts the slice.
        candidate = DispatchingTransformer(case14_config())
    ...
    with torch.inference_mode():
        for trial_index in range(args.trials):
            # A dedicated generator makes each trial deterministic. Candidate
            # and oracle consume the same tensors. Sample-wise CUDA generation
            # is not claimed bitwise-equal to one monolithic batch randn.
            ...
            for sample_index in range(args.batch_size):
                ...
                if candidate is not None:
                    ...
                    candidate_output = candidate.forward_case14_streamed_sample(
                        x, mask
                    )
                    if device.type == "cuda":
                        torch.cuda.synchronize(device)
                    ...
                    trial_failed += accuracy.failed_elements
                    trial_total += accuracy.total_elements
                    trial_max_abs = max(trial_max_abs, accuracy.max_abs_error)
                ...
                output_square_sum += float(
                    output.square().sum(dtype=torch.float64).item()
                )
                output_abs_max = max(
                    output_abs_max, float(output.abs().max().item())
                )
                suffix = (
                    ""
                ...
                print(
                    ...
                    f"sample {sample_index + 1}/{args.batch_size}: "
                    f"complete{suffix}"
                )
                del x, mask, output
            if candidate is not None:
                comparison_trials.append(
                    {
                        "trial": trial_index + 1,
                        "passed": trial_failed == 0,
                        "failed_elements": trial_failed,
                        "total_elements": trial_total,
                        "max_abs_error": trial_max_abs,
                    }
                )
    if device.type == "cuda":
        torch.cuda.synchronize(device)
    ...
    result = {
        ...
        "purpose": (
            "streamed Case-14 FP32 oracle validation; diagnostic proxy timing, "
            "not immutable dense-baseline timing"
        ),
        "config": asdict(config),
        "dtype": "float32",
        "accuracy_trials": args.trials,
    ...
    if candidate is not None:
        result["candidate_comparison"] = {
            "candidate": "DispatchingTransformer FP32-facing Case-14 route",
            "candidate_dtype": "FP32 interface and parameters, FP16 compute",
            "oracle_dtype": "float32",
            "input_contract": "same FP32 sample passed directly to both models",
            ...
            "diagnostic_oracle_speedup": oracle_elapsed / candidate_elapsed,
